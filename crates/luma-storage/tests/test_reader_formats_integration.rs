...
async fn test_reader_service_polymorphic_formats() {
    ...
    let import_service = ImportService::new(db.clone(), file_service, event_bus, job_manager, cache.clone());
    ...
    let (txt_book, txt_file, _) = import_service.import_single_file(&txt_path, device_id).expect("import txt");
    assert_eq!(txt_file.format, DocumentFormat::Txt);

    // Open TXT via ReaderService
    let txt_open = reader.open_document(&txt_book.id, None).await.expect("open txt document");
    ...
    let txt_chapter = reader.get_chapter(&txt_book.id, 0).await.expect("get txt chapter 0");
    assert!(txt_chapter.html_content.contains("Scholarly Epistemology Essay"));
    assert!(txt_chapter.html_content.contains("First paragraph of deep philosophical inquiry"));

    // Search in TXT
    let txt_search = reader.search_document(&txt_book.id, "empiricism").await.expect("search txt");
    ...
    let (md_book, md_file, _) = import_service.import_single_file(&md_path, device_id).expect("import md");
    assert_eq!(md_file.format, DocumentFormat::Md);

    // Open MD via ReaderService
    let md_open = reader.open_document(&md_book.id, None).await.expect("open md document");
    ...
    let md_chapter = reader.get_chapter(&md_book.id, 0).await.expect("get md chapter 0");
    assert!(md_chapter.html_content.contains("<strong>valid inference</strong>"));
    assert!(md_chapter.html_content.contains("Aristotle formalized categorical syllogisms"));

    // Search in MD
    let md_search = reader.search_document(&md_book.id, "syllogisms").await.expect("search md");
    ...
    let (html_book, html_file, _) = import_service.import_single_file(&html_path, device_id).expect("import html");
    assert_eq!(html_file.format, DocumentFormat::Html);

    // Open HTML via ReaderService
    let html_open = reader.open_document(&html_book.id, None).await.expect("open html document");
    ...
    let html_chapter = reader.get_chapter(&html_book.id, 0).await.expect("get html chapter 0");
    assert!(html_chapter.html_content.contains("Wave-Particle Duality"));
    assert!(html_chapter.html_content.contains("both waves and particles"));

    // Search in HTML
    let html_search = reader.search_document(&html_book.id, "duality").await.expect("search html");
